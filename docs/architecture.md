...
## Cost allocation tags: the AWS side and the K8s side

"Cost allocation tags per team" is two mechanisms, not one, operating at two different layers:

- **The AWS-resource side** (every VPC, EC2 instance, EKS cluster, etc. this repo's Terraform creates): `terraform/root.hcl`'s `default_tags` already applies `Project`/`ManagedBy`/`Environment` to everything, and every stack's own `inputs.tags` adds `Tier` (cloud/onprem/shared, since Phase 0) and now `CostCenter` (`"platform-engineering"`, uniformly — see [`terraform/modules/cost-allocation-tags`](../terraform/modules/cost-allocation-tags/)'s README for why one value, not a per-tier split: every resource this repo provisions is genuinely platform-team-owned infrastructure, so a fabricated per-tier `CostCenter` would be decorative, not honest). That module activates those tag keys in Billing/Cost Explorer via `aws_ce_cost_allocation_tag`, so they're actually usable for cost breakdowns instead of sitting inert.
- **The K8s-workload side** (what *other* teams' workloads running on either cluster actually cost): [`policy/gatekeeper-constraints/require-team-label.yaml`](../policy/gatekeeper-constraints/) enforces a `team` label on every `Deployment`, on both clusters, since Phase 7. AWS's Split Cost Allocation Data for Amazon EKS bridges that K8s-level label into the same Cost Explorer/CUR view — up to 50 per-pod Kubernetes labels imported as CUR columns — which is the real per-tenant-team chargeback mechanism this platform enables, once tenant teams actually deploy onto it.

Both are real, current AWS mechanisms — not this repo's own invention. Activating a *new* cost allocation tag requires it to have already been applied to a resource for ~24h first (an eventual-consistency constraint, not an oversight — see the module's README for why it's applied last, from `terraform/live/global/` rather than `terraform/live/dev/`). The EKS Kubernetes-label bridge was *assumed* to be a Billing-console-only toggle with no Terraform resource — checking a live account instead of trusting that assumption found otherwise: the six `aws:eks:*` tags (`Split Cost Allocation Data for EKS`) are ordinary `AWSGenerated`-type cost allocation tags, activated through the same `ce:UpdateCostAllocationTagsStatus` API as the user-defined ones above, and `terraform/modules/cost-allocation-tags` now activates both through the one resource — see that module's README for the full story.
