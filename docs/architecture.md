...
This VM tier lives in its own "on-prem simulation" VPC (`terraform/live/dev/onprem-vpc`, CIDR `10.1.0.0/16`, non-overlapping with the cloud VPC's `10.0.0.0/16` on purpose — see Phase 4's Site-to-Site VPN), reusing the same `vpc` module as the cloud tier with no EKS-specific tags applied.
