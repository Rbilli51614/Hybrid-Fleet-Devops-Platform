...
Both are real, current AWS mechanisms — not this repo's own invention — and both have a genuine manual step neither Terraform nor any provider resource currently covers: activating a *new* cost allocation tag requires it to have already been applied to a resource for ~24h first (an eventual-consistency constraint, not an oversight — see the module's README for why it's applied last, from `terraform/live/global/` rather than `terraform/live/dev/`), and enabling the EKS Kubernetes-label bridge itself is an account-level Billing console toggle with no Terraform resource as of this writing — the same "documented manual step" pattern already used for Amazon Managed Grafana's data sources.
