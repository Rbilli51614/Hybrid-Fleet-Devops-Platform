...
| Observability — Grafana data source | **Not done** | Checked directly against Grafana's own API (a temporary, now-deleted read-only service account token — never the console/SSO session), not just the AWS-level workspace config: `GET /api/datasources` returned `[]`. The workspace's `data_sources = ["PROMETHEUS", "CLOUDWATCH"]` in `terraform/modules/observability` only grants IAM *permission* to query those services — it doesn't create an actual connection inside Grafana. Genuinely the one open manual step left; see [`terraform/modules/observability`](terraform/modules/observability/)'s README. |
