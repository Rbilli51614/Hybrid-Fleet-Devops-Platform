...
The controller itself is installed separately per tier — [`terraform/modules/opa-gatekeeper`](../../../terraform/modules/opa-gatekeeper/) on EKS, [`ansible/roles/opa-gatekeeper`](../../../ansible/roles/opa-gatekeeper/) on the VM tier — same chart version and policy-relevant config (audit interval, etc.) on both, different invocation mechanism because that's how every controller in this repo is split. Replica count intentionally differs (2 on EKS, 1 on the VM tier's single control-plane node — see `ansible/roles/opa-gatekeeper/defaults/main.yml`): an availability/resource-footprint choice, not a policy-parity gap — "parity" here is a claim about the policy content and evaluation behavior, not every Helm value. See [`docs/architecture.md`](../../../docs/architecture.md) for the reasoning.
